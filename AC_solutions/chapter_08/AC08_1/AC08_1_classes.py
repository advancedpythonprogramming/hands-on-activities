...
class Animal(metaclass=ABCMeta):
    def __init__(self, time):
        # This is just a simple way to give an id to each object
    ...
    def dead(self, value):
        ...
        self.__class__.statistics['time_lived'] += self.current_time - \
                                                   self.birth_date

    def next_action_time(self):
        return min(self.next_new_animal_time,
                   self.moment_of_dead,
                   self.next_eating_time)

    def set_next_eating_time(self, food=True):
        if food:
            param = self.metadata['time_for_food']
            self.next_eating_time += int(uniform(param[0], param[1]))
        else:
            self.next_eating_time += 1

    def set_next_new_animal_time(self):
        self.next_new_animal_time += \
            int(expovariate(self.metadata['new_animal']))
    ...
    def eat(self, food, time):
        ...
        if selected.__class__.__name__ in self.metadata['food']:
            print('{0}{2} I am eating a {1}{3}'.format(
                self.__class__.__name__,
                selected.__class__.__name__,
                self.ident, selected.ident))

            if self.time_since_hungry > 0:
                self.__class__.statistics['time_waited_for_food'] += \
                    self.time_since_hungry
            ...
            if self.energy < 0:
                ...
                print('{0}{2} hungry. Found {1}'.format(
                    self.__class__.__name__,
                    selected.__class__.__name__,
                    self.ident))
    ...
    def new_animal(self, time):
        ...
        self.__class__.statistics['current_num_animals'] += 1
        print('Born a new {0} from {1}'.format(
            self.__class__.__name__,
            self.ident))
    ...
    def die(self, reason, time):
        ...
        print('{0}{2} has died because of {1}'.format(
            self.__class__.__name__,
            reason, self.ident))
